...
![eaglercraft](https://raw.githubusercontent.com/konekowo/eaglercraftserver/main/preview.png)
:-:
...
### Client: [https://kone-client.ga/](https://kone-client.ga/) [^1]
...
The worlds are stored in your browser's local storage, **you can export them as EPK files and import them again on all other Eaglercraft sites or clients that also support singleplayer (including kone client).** You can even copy an exported world to an entirely different computer, or send it to a friend, and import it and continue playing with all your progress saved.

## LAN Worlds

### kone client fully supports LAN worlds, you can share your world with any player and they can connect directly to it as if you are running a server in your browser (even if they're using a different Eaglercraft client).
...
### Simply download [stable-download/sp-relay.jar](https://github.com/konekowo/kone-client-full/blob/main/stable-download/sp-relay.jar) and run `java -jar sp-relay.jar`
...
[^2]: Server list is currently being rebuilt
...
2. Download the [stable-download/stable-download.zip](https://github.com/konekowo/kone-client-full/blob/main/stable-download/stable-download.zip) file from this repository
...
5. **To create a link to your site that automatically joins the server,** add a `?server=` variable to the URL, like (for example): [https://kone-client.ga/?server=127.0.0.1:25565](https://kone-client.ga/?server=127.0.0.1:25565) will automatically join `ws://127.0.0.1:25565/` as soon as the player finishes setting their username and skin
...
**To develop a plugin, download [stable-download/java/bungee_command/bungee_dist.jar](https://github.com/konekowo/kone-client-full/blob/main/stable-download/java/bungee_command/bungee-dist.jar) and add it to the Build Path of your Java IDE. Develop the plugin just like a regular BungeeCord plugin, see [EaglerMOTD](https://github.com/lax1dude/eaglercraft-motd/) for an example.**
...
- **[net.md_5.bungee.api.event.WebsocketMOTDEvent](https://github.com/konekowo/kone-client-full/blob/main/eaglercraftbungee/src/main/java/net/md_5/bungee/api/event/WebsocketMOTDEvent.java)**: Triggered when a client or website requests the MOTD

- **[net.md_5.bungee.api.event.WebsocketQueryEvent](https://github.com/konekowo/kone-client-full/blob/main/eaglercraftbungee/src/main/java/net/md_5/bungee/api/event/WebsocketQueryEvent.java)**: Triggered when a client or website requests a query. This happens when a site opens a text WebSocket to a listener and sends a single string `Accept: <query>` packet. Can be used to provide additional custom statistics to server list sites supporting integrated WebSocket queries
...
- To make a custom resource pack for your site, clone this repository and edit the files in [lwjgl-rundir/resources](https://github.com/konekowo/kone-client-full/tree/main/lwjgl-rundir/resources).
- When you are done, navigate to [epkcompiler/](https://github.com/konekowo/kone-client-full/tree/main/epkcompiler) and double-click `run.bat`. Wait for the window to say `Press any key to continue...` and close it. Then, go to `../javascript` in the repository and copy `javascript/assets.epk` to the `assets.epk` on your website
