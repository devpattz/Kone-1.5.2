...
public class GuiOptions extends GuiScreen {
	 ...
	 */
	private static final EnumOptions[] relevantOptions = new EnumOptions[] { EnumOptions.MUSIC, EnumOptions.SOUND, EnumOptions.INVERT_MOUSE, EnumOptions.SENSITIVITY, EnumOptions.FOV, EnumOptions.DIFFICULTY, EnumOptions.ANTIALIASING, EnumOptions.KEY_STROKES_MOD };
	...
	public void initGui() {
		...
		for (int var5 = 0; var5 < var4; ++var5) {
			...
			if (var6.getEnumFloat()) {
				this.buttonList.add(new GuiSlider(var6.returnEnumOrdinal(), this.width / 2 - 152 + var2 % 2 * 160, this.height / 6 - 12 + 24 * (var2 >> 1), var6, this.options.getKeyBinding(var6), this.options.getOptionFloatValue(var6)));
			} else {
				GuiSmallButton var7 = new GuiSmallButton(var6.returnEnumOrdinal(), this.width / 2 - 152 + var2 % 2 * 160, this.height / 6 - 12 + 24 * (var2 >> 1), var6, this.options.getKeyBinding(var6));

				if (this.mc.gameSettings.keyStrokes){
					//System.out.println("true");
					if (var7.displayString.startsWith("Key Strokes Mod")){
						var7.displayString = "Key Strokes Mod: ON";
					}
				}
				else {
					//System.out.println("false");
					if (var7.displayString.startsWith("Key Strokes Mod")){
						var7.displayString = "Key Strokes Mod: OFF";
					}
				}
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		if (par1GuiButton.enabled) {
			if (par1GuiButton.id < 100 && par1GuiButton instanceof GuiSmallButton) {
				...
				par1GuiButton.displayString = this.mc.gameSettings.getKeyBinding(EnumOptions.getEnumOptions(par1GuiButton.id));
				//System.out.println(this.options.getKeyBinding(EnumOptions.getEnumOptions(par1GuiButton.id)));
				if (this.mc.gameSettings.keyStrokes){
					//System.out.println("true");
					if (par1GuiButton.displayString.startsWith("Key Strokes Mod")){
						par1GuiButton.displayString = "Key Strokes Mod: ON";
					}
				}
				else {
					//System.out.println("false");
					if (par1GuiButton.displayString.startsWith("Key Strokes Mod")){
						par1GuiButton.displayString = "Key Strokes Mod: OFF";
					}
				}
